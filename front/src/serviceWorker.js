...
will only see deployed updates on subsequent visits to a page, after all the existing tabs open on the page have been closed, since previously cached
resources are updated in the background. To learn more about the benefits of this model and instructions on how to opt-in, read https://bit.ly/CRA-PWA */
...
export function register(config) {
  if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
    const publicUrl = new URL(process.env.PUBLIC_URL, window.location.href) // The URL constructor is available in all browsers that support SW.
    ...
    window.addEventListener('load', () => {
      ...
      if (isLocalhost) {
        checkValidServiceWorker(swUrl, config) // This is running on localhost. Let's check if a service worker still exists or not.
          ...
          if (installingWorker.state === 'installed') {
            if (navigator.serviceWorker.controller) {
              ...
              if (config && config.onUpdate) {
                config.onUpdate(registration) // Execute callback
              }
            } else {
